...
var PHASER_STATE_PATH = './node_modules/phaser-state-transition/dist/';
...
function copyPhaserTransition() {

    var srcList = ['phaser-state-transition.min.js'];
    srcList = srcList.map(function(file) {
        return PHASER_STATE_PATH + file;
    });
    return gulp.src(srcList)
        .pipe(gulp.dest(SCRIPTS_PATH));
...
gulp.task('copyPhaserTransition', ['copyStatic'], copyPhaserTransition);
gulp.task('build', ['copyPhaser', 'copyPhaserTransition'], build);
